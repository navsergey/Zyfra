...
export class TextFormatterService {
  ...
  formatMessageText(text: string): string {
    ...
    // Удаляем все блоки с документами в квадратных скобках
    // Поддерживает сложные случаи с экранированными кавычками: [Документ \"filename.pdf\"]
    // И множественные документы: [Документ \"file1.pdf\"], [Документ \"file2.pdf\"]
    const documentPattern = /\[Документ\s+\\?"[^"]*\\?"[^\]]*\]|\[Документ[^\]]*\]/g;
